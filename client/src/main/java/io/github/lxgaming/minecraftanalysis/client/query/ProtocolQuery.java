...
package io.github.lxgaming.minecraftanalysis.client.query;
...
public class ProtocolQuery extends Query {
    ...
    public void execute() throws Exception {
        JsonArray jsonArray;
        if (MinecraftAnalysis.getInstance().getVersion().getId().equals("1.14.4")) {
            jsonArray = processPackets();
        } else {
            jsonArray = processFlows();
        }
        
        MinecraftAnalysis.getInstance().write("protocol", jsonArray);
    }
